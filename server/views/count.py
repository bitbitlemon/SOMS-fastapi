...
from server.schemas.count import AchievementScoreRank
...
# async def _count_achievement(db: Session = Depends(get_db), user: User = Depends(get_current_user_admin)) -> APIResponse:
async def _count_achievement(achievement: AchievementScoreRank, db: Session = Depends(get_db)) -> APIResponse:
    """获取成果总分排行榜"""
    try:
